...
export interface BaseLogEntry {
  ...
  /** ISO 8601 timestamp — auto-generated by emitLog() if not provided */
  timestamp?: string
 ...
 * Auto-generates timestamp if not provided — centralizes timestamp logic
 * to eliminate boilerplate and prevent stale timestamps in cached contexts.
 * next-axiom captures console output automatically.
 *
 * @param entry - The log entry to emit (API, PageView, UserAction, or SystemEvent)
 */
export function emitLog(entry: LogEntry): void {
  const enrichedEntry = {
    // Default timestamp — generated at emit time, caller can override
    timestamp: new Date().toISOString(),
    ...entry,
    // Environment enrichment — always override caller values
